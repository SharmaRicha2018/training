...
func GetOrchestrator(resource string,
	version string,
	action string,
	bucketId string) (*workflow.Orchestrator, *ratelimiter.RateLimiter, error) {
	...
	orchestratorVersion, ratelimiter, gerr := versionmanager.Get(resource, version, action, bucketId)
	if gerr != nil {
	...
	return &orchestrator, ratelimiter, nil
