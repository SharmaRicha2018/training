...
func (c *CentralCacheImpl) GetBatch(keys []string, serialize bool, compress bool) (map[string]*BatchItemResp, error) {
	resMap := make(map[string]*BatchItemResp, len(keys))
	keyString := ""
	for _, v := range keys {
		resMap[v] = nil
	...
	err = json.Unmarshal(res.Body, cr)
	if err != nil {
		logger.Error("GetBatch : Unmarshalling error. Error - " + err.Error())
		return nil, ErrInvalidCacheResponse
	}
	for _, v := range cr.Data {
		item := new(BatchItemResp)
		item.Key = v.Key
		item.Value = v.Value
		item.Error = v.Error
