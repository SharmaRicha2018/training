...
// find all
func (obj *mongoDriver) FindAllN(collection string, query map[string]interface{}, ret interface{}) (aerr *MongodbError) {
	err := obj.conn.C(collection).Find(bson.M(query)).All(ret)
	if err != nil {
...
// find all and sort
func (obj *mongoDriver) FindAllNSort(collection string, query map[string]interface{}, ret interface{}, fields string) (aerr *MongodbError) {
	err := obj.conn.C(collection).Find(bson.M(query)).Sort(fields).All(ret)
	if err != nil {
		return getErrObj(ERR_FINDALL_FAILURE, err.Error())
	} else {
		return nil
