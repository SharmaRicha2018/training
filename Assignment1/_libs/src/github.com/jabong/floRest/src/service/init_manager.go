...
func InitVersionManager() {
	vmap := versionmanager.VersionMap{
		...
		}: versionmanager.NewFullVersion(createServiceOrchestrator(), nil),
		versionmanager.Version{
			Resource: constants.HEALTHCHECKAPI,
			Version:  "",
			Action:   "GET",
			BucketId: constants.ORCHESTRATOR_BUCKET_DEFAULT_VALUE,
		}: versionmanager.NewFullVersion(createHealthCheckOrchestrator(), nil),
	}
...
func addApiVersions(vmap versionmanager.VersionMap) {
	for _, apiInstance := range apiList {
		r := apiInstance.GetRateLimiter()
		vmap[apiInstance.GetVersion()] = versionmanager.NewFullVersion(apiInstance.GetOrchestrator(), &r)
	}
}
